...
def compute_video_fidelity(video_dir, bench_csv, device='cuda:0', num_gpus=8,
    ...
    logger.info(f'Video Fidelity evaluation: {len(entries)} videos')
    ...
    overall_stats = compute_statistics(results, 'video_fidelity',
                                       valid_fn=lambda x: x is not None)
    per_task_stats = compute_per_task_statistics(results, 'video_fidelity',
                                                 valid_fn=lambda x: x is not None)

    logger.info(f'Video Fidelity done: mean={overall_stats["mean"]}, '
...
def main():
    ...
    overall_stats, per_task_stats, results = compute_video_fidelity(
        video_dir=args.video_dir,
        bench_csv=bench_csv,
        path_cfg=path_cfg,
    )

    output_path = args.output or os.path.join(args.video_dir, 'video_fidelity_results.json')
    save_json({'metric': 'video_fidelity', 'video_dir': args.video_dir,
