...
import torch
import torchvision
from .yoloface_v5l import Model as YoloFaceModel
...
def non_max_suppression_face(prediction, conf_thres=0.5, iou_thres=0.45, classes=None, agnostic=False, labels=()):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        i = torchvision.ops.nms(boxes.cpu(), scores.cpu(), iou_thres).to(boxes.device)  # NMS (CPU fallback for ROCm)
...
class YoloFace():
    def __init__(self, pt_path='checkpoints/yolov5m-face.pt', confThreshold=0.5, nmsThreshold=0.45, device='cuda'):
        ...
        if pt_path.endswith('.pt'):
            self.model = torch.jit.load(pt_path).to(self.test_device).half()
            self.pt_model = True
        else:
            self.model = YoloFaceModel(pt_path).eval().to(self.test_device).half()
        self.last_w = 416
        self.last_h = 416
        self.grids = None
    ...
    def detect(self, srcimg):
        ...
        # Run inference
        img = torch.from_numpy(img).to(self.test_device, non_blocking=False).permute(2,0,1).unsqueeze(0)
        img = img.half() / 255.0  # uint8 to fp16/32  0-1
        # if img.ndimension() == 3:
        #     img = img.unsqueeze(0)
        ...
        if h1 != self.last_h or w1 != self.last_w or self.grids is None:
            ...
            for scale in [8,16,32]:
                ...
                grid = torch.stack((xv, yv), 2).view((1,1,ny, nx, 2)).half()
                grids.append(grid.to(self.test_device, non_blocking=False).half())
            self.grids = grids
            self.last_w = w1
            self.last_h = h1
        ...
        # t1 = time.time()
        if self.pt_model:
            pred = self.model(img.half(), self.grids).float()
        else:
            pred = self.model(img.half(), *self.grids).float()
        # torch.cuda.synchronize()
        # t2 = time.time()
        ...
        # Apply NMS
        det = non_max_suppression_face(pred, self.conf_thres, self.iou_thres)[0].cpu()
        # torch.cuda.synchronize()
        # t3 = time.time()
        # print(t2 - t1, t3 - t2)
        # print(pred.shape)
...
if __name__ == '__main__':
    ...
    imgpath = 'data/warmup.png'

    yoloface = YoloFace(pt_path='data/models/yoloface_v5l.bin')
    yoloface.model = torch.compile(yoloface.model, mode='reduce-overhead', dynamic=True)
    ...
    torch.cuda.synchronize()
    t2 = time.time()
    print('total time: {} ms'.format((t2 - t1) / 100 * 1000))
